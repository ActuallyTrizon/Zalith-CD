...
class LaunchArgs(
    ...
    private fun getJavaArgs(): List<String> {
        ...
        if (com.movtery.zalithlauncher.setting.AllSettings.forceOpenGl.getValue()) {
            argsList.add("-Dorg.lwjgl.vulkan.disable=true")
        }
...
//        }
        ...
        val resolved = JSONUtils.insertJSONValueList(minecraftArgs.toTypedArray<String>(), varArgMap)

        // Minecraft 1.21.4+ (26.2+) version JSON sets native extraction paths relative to
        // ${natives_directory} which resolves to the APK lib folder — read-only on Android API 29+.
        // Redirect them to the writable cache directory so LWJGL/JNA/Netty can extract there.
        return resolved.map { arg ->
            when {
                arg.startsWith("-Djna.tmpdir=") -> {
                    val dir = File(PathManager.DIR_CACHE, "jna").also { it.mkdirs() }
                    "-Djna.tmpdir=${dir.absolutePath}"
                }
                arg.startsWith("-Dorg.lwjgl.system.SharedLibraryExtractPath=") -> {
                    val dir = File(PathManager.DIR_CACHE, "lwjgl").also { it.mkdirs() }
                    "-Dorg.lwjgl.system.SharedLibraryExtractPath=${dir.absolutePath}"
                }
                arg.startsWith("-Dio.netty.native.workdir=") -> {
                    val dir = File(PathManager.DIR_CACHE, "netty").also { it.mkdirs() }
                    "-Dio.netty.native.workdir=${dir.absolutePath}"
                }
                arg.startsWith("-Djava.library.path=") ->
                    "-Djava.library.path=${PathManager.DIR_NATIVE_LIB}"
                else -> arg
            }
        }.toTypedArray()
    }
